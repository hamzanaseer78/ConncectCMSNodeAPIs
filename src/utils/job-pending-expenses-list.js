...
function formatPendingExpenseListRow(job) {
...
function buildPendingExpenseJobWhere(auth, query = {}, options = {}) {
  ...
  const minAmount = parseFloatFilter(q.minAmount ?? q.minTotalCost);
  const maxAmount = parseFloatFilter(q.maxAmount ?? q.maxTotalCost);
...
function buildPendingExpenseListOrderBy(query = {}) {
...
function getAvailablePendingExpenseListFilters() {
  return [
...
function buildEmptyPendingExpensesResponse(service, pagination, settings) {
  return {
    mode: service.mode,
    expensesEnabled: settings?.allowaddexpenses === true,
    data: [],
    summary: {
      totalPendingJobs: 0,
      totalRecords: 0
...
module.exports = {
  ...
  PENDING_EXPENSE_JOB_INCLUDE: COLLECTION_LIST_JOB_INCLUDE
};
