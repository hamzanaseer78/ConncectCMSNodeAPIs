...
const { BULK_UPLOAD_EXTENSIONS } = require("../config/bulk-upload-common");

function detectFileType(filename) {
  const ext = path.extname(filename || "").toLowerCase();
  if (BULK_UPLOAD_EXTENSIONS.has(ext) && ext !== ".pdf") return "excel";
...
function buildTemplateWorkbookBuffer(columns, templateRow, sheetName = "Sheet1") {
  const headers = columns.map((col) => col.label);
  const workbook = XLSX.utils.book_new();
  const sheet = XLSX.utils.aoa_to_sheet([headers, templateRow]);
